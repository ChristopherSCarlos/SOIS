<div class="divide-y divide-gray-800" x-data="{ show: false }">
    <nav id="orgNav" class="flex items-center px-3 py-2 shadow-lg">
                <div>
    ...
    @foreach($pageTableDate as $PageData)
        @if($urlslug == $PageData->slug or $urlslug == '')
            ...
            <main id="frontpagebg" class="bg-gray-100 p-12 min-h-screen sm:w-12/12 md:w-12/12 lg:w-12/12 bg-cover bg-center" style="background-image: url(image/svg/backgroudn.svg)">
                <div id="FrontPageFlex">
                    <div id="FrontPageLogoContainer">
    ...
    @foreach($pageOrgData as $OrgData)
        @if($urlslug == $OrgData->organization_slug)
            ...
            <aside class="sm:hidden bg-gray-900 text-gray-700 divide-y divide-gray-700 divide-dashed sm:w-4/12 md:w-3/12 lg:w-2/12">
                {{-- Desktop Web View --}}
                <ul class="hidden text-gray-200 text-xs sm:block sm:text-left">
                    @foreach($pageOrgData as $OrgDataItems)
                        <a href="{{ url('/'.$OrgDataItems->organization_slug) }}" id="navLinks">
                            {{ $OrgDataItems->organization_name }}
                        </a>
                    @endforeach
                </ul>
                {{-- Mobile Web View --}}
                <div :class="show ? 'block' : 'hidden'" class="pb-3 divide-y divide-gray-800 block sm:hidden">
                    <ul class="text-gray-200 text-xs">
                        @foreach($pageOrgData as $OrgDataItems)
                            <a href="{{ url('/'.$OrgDataItems->organization_slug) }}" id="orgNavLinks" class="cursor-pointer px-4 py-2 hover:bg-gray-800">
                                <li class="cursor-pointer px-4 py-2 hover:bg-gray-800">
                                    {{ $OrgDataItems->organization_name }}
                                </li>
                            </a>
            ...
            <p id="eventTitle" class="text-center bg-transparent mt-0">Events</p>

<div id="eventCard" class="flex flex-wrap align-center justify-center">
@foreach($pageEventData as $events)
    <div id="card" class="max-w-sm rounded overflow-hidden shadow-lg bg-red-500 m-5">
        <img class="w-full" src="{{ asset('/image/carousel3.jpg') }}" alt="Sunset in the mountains">
        <div class="px-6 py-4">
            <div class="font-bold text-xl mb-2">{{$events->event_name}}</div>
            <p class="text-gray-700 text-base">
                {{$events->event_details}}
            </p>
        </div>
        <div class="px-6 pt-4 pb-2 flex flex-wrap">
            <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">{{$events->created_at}}</span>
            <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">{{$events->updated_at}}</span>
        </div>
    </div>
@endforeach
</div>
