...
class Users extends Component
{
    ...
    {
        $this->isUserRoleAuthId = DB::table('role_user')
                                    ->orderBy('id','asc')
                                    ->get()
                                    ->pluck('user_id');
        $this->isUserRoleAuthArray = array($this->isUserRoleAuthId); 
        return $this->isUserRoleAuthId;
    }
    public function isUserHaveTeam()
    {
        $this->isUserTeamAuthId = DB::table('teams')
                                    ->orderBy('id','asc')
                                    ->get()
                                    ->pluck('user_id');
        $this->isUserTeamAuthArray = array($this->isUserTeamAuthId); 
        return $this->isUserTeamAuthId;
    }
    
    public function deleteShowUserTeam($id)
    {
        $this->userId = $id;
        $this->modalDeleteTeamsFormVisible = true;
    }
    public function deleteTeamUser()
    {
        $this->userTeamDataForDeletion = DB::table('teams')->where('user_id','=',$this->userId)->first();
        $this->userTeamDataForDeletionId = $this->userTeamDataForDeletion->id; 
        $this->userTeamDataForDeletionUserId = $this->userTeamDataForDeletion->user_id;
        User::where('id',$this->userTeamDataForDeletionUserId)->update(['current_team_id' => $this->userTeamDataForDeletionUserNull]);
        Team::destroy($this->userTeamDataForDeletionId);
        $this->modalDeleteTeamsFormVisible = false;
        $this->reset();
    ...
    public function deleteUserData()
    {
        ...
        // dd($this->userId);
        // $this->usersDeleteRoleData = User::find($this->userId);
        // $this->usersDeleteRoleDataId = $this->user->roles->first();
        // dd($this->usersDeleteRoleDataId->id);

        $this->usersDeleteRoleData = DB::table("role_user")->where("user_id","=",$this->userId)->delete();
        $this->usersDeleteRoleData = DB::table("organization_user")->where("user_id","=",$this->userId)->delete();
        $this->usersDeleteRoleData = DB::table("teams")->where("user_id","=",$this->userId)->delete();
        User::destroy($this->userId);



        // dd($this->usersDeleteRoleData);


        // $this->usersDeleteRoleData = User::find($this->userId);
        // $this->usersDeleteRoleDataId = $this->usersDeleteRoleData;
        // $this->usersOrganizationRoleData = User::find($this->userId)->organization;
        // $this->usersTeamRoleData = User::find($this->userId)->teams;
        // dd($this->usersDeleteRoleDataId);
        // $this->userDeleteId->roles()->detach($role_id);
    ...
    public function render()
    {
        return view('livewire.users',[
            ...
            'isUsersHaveRole' => $this->isUserHaveRole(),
            'isUsersHaveTeam' => $this->isUserHaveTeam(),
        ]);
    }
}
